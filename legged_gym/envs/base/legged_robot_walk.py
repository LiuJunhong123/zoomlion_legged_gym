...
class LeggedRobotWalk(BaseTask):
    ...
    def post_physics_step(self):
        ...
        # TODO: fix use gait_frequency bug
        # self.gait_process = self.gait_process + self.dt * self.gait_frequency
        self.gait_process += 1 # * (self.gait_frequency > 1.0e-8).float()
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.commands.switch_gait:
    ...
    def generate_gait_time(self, env_ids):
        if len(env_ids) == 0:
            return
        ...
        self.gait_time[env_ids] = torch.cumsum(scaled_tensor, dim=1).int()
    ...
    def _resample_stand_command(self, env_ids):
        ...
        self.gait_frequency[env_ids] = torch.zeros(len(env_ids), device=self.device)
    ...
    def _resample_walk_sagittal_command(self, env_ids):
        ...
        self.cycle_time[env_ids] = torch_rand_float(self.cfg.rewards.cycle_time * self.cfg.rewards.cycle_range[0], self.cfg.rewards.cycle_time * self.cfg.rewards.cycle_range[1], \
                                                    (len(env_ids), 1), device=self.device).squeeze(1)
        self.gait_frequency[env_ids] = 1 / self.cycle_time[env_ids]
    ...
    def _resample_commands(self, env_ids):
        ...
        self.commands[still_envs, :3] = 0.0
    ...
    def _init_buffers(self):
        ...
        # self.gait_process = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.gait_process = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.gait_frequency = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
