...
class LeggedRobotStand(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        if self.cfg.env.ref_state_init:
            motion_files = []
            for file_path in self.cfg.env.amp_motion_files:
                motion_files.append(file_path.format(LEGGED_GYM_ROOT_DIR=LEGGED_GYM_ROOT_DIR))
    ...
    def update_obs_latency_buffer(self):
        ...
        if self.cfg.domain_rand.randomize_obs_imu_latency:
            ...
            self.obs_imu_latency_buffer[:,:,0] = torch.cat((self.base_ang_vel * self.obs_scales.ang_vel, self.projected_gravity * self.obs_scales.gravity), 1).clone()
    ...
    def reset_idx(self, env_ids):
        ...
        self.gait_process[env_ids] = 0  # TODO: cycle_time, vel_i need reset ?
        ...
        if self.cfg.commands.curriculum:
            ...
            self.extras["episode"]["max_command_z"] = self.command_ranges["ang_vel_yaw"][1]
            self.extras["episode"]["min_command_z"] = self.command_ranges["ang_vel_yaw"][0]
    ...
    def _resample_commands(self, env_ids):
        ...
        # TODO: to be greater code
        self.vel_i[env_ids] = (torch.abs(self.commands[env_ids, 0]) / self.cfg.rewards.track_vel_threshold).long()
        self.cycle_i[env_ids] = torch.where(self.vel_i[env_ids] >= len(self.cfg.rewards.cycle_time),
                                            len(self.cfg.rewards.cycle_time) - 1, self.vel_i[env_ids])
        self.cycle_val[env_ids] = self.reward_cycle[self.cycle_i[env_ids]]

        self.swing_i[env_ids] = torch.where(self.vel_i[env_ids] >= len(self.cfg.rewards.target_swing),
                                            len(self.cfg.rewards.target_swing) - 1, self.vel_i[env_ids])
        self.swing_val[env_ids] = self.reward_swing[self.swing_i[env_ids]]

        self.elbow_i[env_ids] = torch.where(self.vel_i[env_ids] >= len(self.cfg.rewards.target_elbow),
                                            len(self.cfg.rewards.target_elbow) - 1, self.vel_i[env_ids])
        self.elbow_val[env_ids] = self.reward_elbow[self.elbow_i[env_ids]]

        self.feet_height_i[env_ids] = torch.where(self.vel_i[env_ids] >= len(self.cfg.rewards.target_feet_height),
                                            len(self.cfg.rewards.target_feet_height) - 1, self.vel_i[env_ids])
        self.feet_height_val[env_ids] = self.reward_feet_height[self.feet_height_i[env_ids]]

        self.stance_threshold_i[env_ids] = torch.where(self.vel_i[env_ids] >= len(self.cfg.rewards.stance_threshold),
                                            len(self.cfg.rewards.stance_threshold) - 1, self.vel_i[env_ids])
        self.stance_threshold_val[env_ids] = self.reward_stance_threshold[self.stance_threshold_i[env_ids]]

        lower = self.cycle_val[env_ids] * self.cfg.rewards.cycle_range[0]
        upper = self.cycle_val[env_ids] * self.cfg.rewards.cycle_range[1]
        self.cycle_time[env_ids] = torch.rand(len(env_ids), device=self.device) * (upper - lower) + lower
        # self.cycle_time[env_ids] = torch_rand_float(self.cfg.rewards.cycle_time * self.cfg.rewards.cycle_range[0], self.cfg.rewards.cycle_time * self.cfg.rewards.cycle_range[1], \
        #                                             (len(env_ids), 1), device=self.device).squeeze(1)
        # set low commands
        low_commands_envs = env_ids[torch.randperm(len(env_ids))[: int(self.cfg.commands.low_commands_proportion * len(env_ids))]]
        low_commands_threshold = self.cfg.commands.low_commands_threshold
        self.commands[low_commands_envs, 0] = torch_rand_float(-low_commands_threshold, low_commands_threshold, (len(low_commands_envs), 1), device=self.device).squeeze(1)
        self.commands[low_commands_envs, 1] = torch_rand_float(-low_commands_threshold, low_commands_threshold, (len(low_commands_envs), 1), device=self.device).squeeze(1)
        if self.cfg.commands.heading_command:
            self.commands[low_commands_envs, 3] = torch_rand_float(-low_commands_threshold, low_commands_threshold, (len(low_commands_envs), 1), device=self.device).squeeze(1)
        else:
            self.commands[low_commands_envs, 2] = torch_rand_float(-low_commands_threshold, low_commands_threshold, (len(low_commands_envs), 1), device=self.device).squeeze(1)
        # set commands to zero
        stand_envs = env_ids[torch.randperm(len(env_ids))[: int(self.cfg.commands.stand_commands_proportion * len(env_ids))]]
        self.commands[stand_envs, :3] = 0.0
        stand_ids = torch.norm(self.commands[env_ids, :3], dim=1) < self.cfg.commands.stand_commands_threshold
        self.commands[env_ids, :3][stand_ids] = 0.0
    ...
    def update_command_curriculum(self, env_ids):
        ...
        if torch.mean(self.episode_sums["track_lin_vel_xy"][env_ids]) / self.max_episode_length > \
            self.cfg.commands.curriculum_lin_threshold * self.reward_scales["track_lin_vel_xy"]:
            self.command_ranges["lin_vel_x"][0] = np.clip(self.command_ranges["lin_vel_x"][0] - 0.1, self.cfg.commands.ranges.lin_vel_x_limit[0], 0.)
            self.command_ranges["lin_vel_x"][1] = np.clip(self.command_ranges["lin_vel_x"][1] + 0.1, 0., self.cfg.commands.ranges.lin_vel_x_limit[1])
            self.command_ranges["lin_vel_y"][0] = np.clip(self.command_ranges["lin_vel_y"][0] - 0.1, self.cfg.commands.ranges.lin_vel_y_limit[0], 0.)
            self.command_ranges["lin_vel_y"][1] = np.clip(self.command_ranges["lin_vel_y"][1] + 0.1, 0., self.cfg.commands.ranges.lin_vel_y_limit[1])

        if torch.mean(self.episode_sums["track_ang_vel_z"][env_ids]) / self.max_episode_length > \
            self.cfg.commands.curriculum_ang_threshold * self.reward_scales["track_ang_vel_z"]:
            self.command_ranges["ang_vel_yaw"][0] = np.clip(self.command_ranges["ang_vel_yaw"][0] - 0.1, self.cfg.commands.ranges.ang_vel_yaw_limit[0], 0.)
            self.command_ranges["ang_vel_yaw"][1] = np.clip(self.command_ranges["ang_vel_yaw"][1] + 0.1, 0., self.cfg.commands.ranges.ang_vel_yaw_limit[1])  
    ...
    def _init_buffers(self):
        ...
        self.gait_process = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.vel_i = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.cycle_i = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.cycle_val = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.reward_cycle = torch.tensor(self.cfg.rewards.cycle_time, device=self.device, requires_grad=False)
        self.swing_i = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.swing_val = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.reward_swing = torch.tensor(self.cfg.rewards.target_swing, device=self.device, requires_grad=False)
        self.elbow_i = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.elbow_val = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.reward_elbow = torch.tensor(self.cfg.rewards.target_elbow, device=self.device, requires_grad=False)
        self.feet_height_i = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.feet_height_val = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.reward_feet_height = torch.tensor(self.cfg.rewards.target_feet_height, device=self.device, requires_grad=False)
        self.stance_threshold_i = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.stance_threshold_val = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.reward_stance_threshold = torch.tensor(self.cfg.rewards.stance_threshold, device=self.device, requires_grad=False)
        self.cycle_time = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _create_envs(self):
        ...
        feet_names = []
        for name in self.cfg.asset.feet_name:
            feet_names.extend([s for s in body_names if name in s])
        ...
        arms_names = []
        for name in self.cfg.asset.arms_name:
            arms_names.extend([s for s in self.dof_names if name in s])
        swing_arms_names = []
        for name in self.cfg.asset.swing_arms_name:
            swing_arms_names.extend([s for s in self.dof_names if name in s])
        shoulders_names = []
        for name in self.cfg.asset.shoulders_name:
            shoulders_names.extend([s for s in self.dof_names if name in s])
        elbows_names = []
        for name in self.cfg.asset.elbows_name:
            elbows_names.extend([s for s in self.dof_names if name in s])
        waists_names = []
        for name in self.cfg.asset.waists_name:
            waists_names.extend([s for s in self.dof_names if name in s])
        legs_names = []
        for name in self.cfg.asset.legs_name:
            legs_names.extend([s for s in self.dof_names if name in s])
        stand_names = []
        for name in self.cfg.asset.stand_name:
            stand_names.extend([s for s in self.dof_names if name in s])

        left_arm_dof = []
        for name in self.cfg.asset.left_arm:
            left_arm_dof.extend([s for s in self.dof_names if name in s])
        right_arm_dof = []
        for name in self.cfg.asset.right_arm:
            right_arm_dof.extend([s for s in self.dof_names if name in s])
        waist_dof = []
        for name in self.cfg.asset.waist:
            waist_dof.extend([s for s in self.dof_names if name in s])
        left_leg_dof = []
        for name in self.cfg.asset.left_leg:
            left_leg_dof.extend([s for s in self.dof_names if name in s])
        right_leg_dof = []
        for name in self.cfg.asset.right_leg:
            right_leg_dof.extend([s for s in self.dof_names if name in s])

        hand_body = []
        for name in self.cfg.asset.hand_body:
            hand_body.extend([s for s in body_names if name in s])
        feet_body = []
        for name in self.cfg.asset.feet_body:
            feet_body.extend([s for s in body_names if name in s])
        ...
        self.hand_body_ids = torch.zeros(len(hand_body), dtype=torch.long, device=self.device, requires_grad=False)
        for i in range(len(hand_body)):
            self.hand_body_ids[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.actor_handles[0], hand_body[i])
        self.feet_body_ids = torch.zeros(len(feet_body), dtype=torch.long, device=self.device, requires_grad=False)
        for i in range(len(feet_body)):
            self.feet_body_ids[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.actor_handles[0], feet_body[i])

        self.arms_indices = []
        self.swing_arms_indices = []
        self.shoulders_indices = []
        self.elbows_indices = []
        self.waists_indices = []
        self.legs_indices = []
        self.stand_indices = []
        ...
        for i in range(self.num_dofs):
            name = self.dof_names[i]
            if name in arms_names:
                self.arms_indices.append(i)
            if name in swing_arms_names:
                self.swing_arms_indices.append(i)
            if name in shoulders_names:
                self.shoulders_indices.append(i)
            if name in elbows_names:
                self.elbows_indices.append(i)
            if name in waists_names:
                self.waists_indices.append(i)
            if name in legs_names:
                self.legs_indices.append(i)
            if name in stand_names:
                self.stand_indices.append(i)

            if name in left_arm_dof:
                self.left_arm_ids.append(i)
            if name in right_arm_dof:
                self.right_arm_ids.append(i)
            if name in waist_dof:
                self.waist_ids.append(i)
            if name in left_leg_dof:
                self.left_leg_ids.append(i)
            if name in right_leg_dof:
                self.right_leg_ids.append(i)

        print(f"self.arms_indices: {self.arms_indices}")
        print(f"self.swing_arms_indices: {self.swing_arms_indices}")
        print(f"self.shoulders_indices: {self.shoulders_indices}")
        print(f"self.elbows_indices: {self.elbows_indices}")
        print(f"self.waists_indices: {self.waists_indices}")
        print(f"self.legs_indices: {self.legs_indices}")
        print(f"self.stand_indices: {self.stand_indices}")

        print(f"self.left_arm_ids: {self.left_arm_ids}")
        print(f"self.right_arm_ids: {self.right_arm_ids}")
        print(f"self.waist_ids: {self.waist_ids}")
        print(f"self.left_leg_ids: {self.left_leg_ids}")
        print(f"self.right_leg_ids: {self.right_leg_ids}")

        print(f"self.hand_body_ids: {self.hand_body_ids}")
        print(f"self.feet_body_ids: {self.feet_body_ids}")
